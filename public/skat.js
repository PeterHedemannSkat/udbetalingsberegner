...
var inputTypes = {
  string: {
    ...
    minlength: function(string, length) {
      return string.toString().length >= Number(length);
    },
  ...
  dkDate: {
    validate: function(value) {
      ...
      return helper.convertToDate(value) ? true : false;
    },
    mindate: function(date, minDateString) {
      if (!date || !minDateString) {
        return true;
      }
      if (typeof date === "string") {
        date = helper.convertToDate(date);
      }
      var minDate = helper.convertToDate(minDateString);

      return date && minDate && date >= minDate;
    },
    maxdate: function(date, maxDateString) {
      ...
      var maxDate = helper.convertToDate(maxDateString);
  ...
  dkDecimal: {
    validate: function(value) {
      return helper.checkDkDecimal(value);
    },
    maxexclusive: function(value, maxValue) {
      if (!value) {
        return true;
      }

      var numberValue = helper.convertToNumber(value);
      var numberMax = helper.convertToNumber(maxValue);
      if (numberMax === NaN || numberValue === NaN) {
        return false;
      }

      return numberValue < numberMax;
    },
    maxinclusive: function(value, maxValue) {
      if (!value) {
        return true;
      }
    ...
    mininclusive: function(value, minValue) {
      if (!value) {
        return true;
      }

      var numberValue = helper.convertToNumber(value);
      var numberMin = helper.convertToNumber(minValue);
      if (numberMin === NaN || numberValue === NaN) {
        return false;
      }

      return numberValue >= numberMin;
    },
    minexclusive: function(value, minValue) {
      if (!value) {
        return true;
      }
  ...
  dkInteger: {
    validate: function(value) {
      return !value || helper.checkInteger(value);
    },
    maxexclusive: function(value, maxValue) {
      if (!value) {
        return true;
      ...
      var numberValue = helper.convertToNumber(value);
      var maxNumberValue = helper.convertToNumber(maxValue);
      return numberValue < maxNumberValue;
    },
    maxinclusive: function(value, maxValue) {
      if (!value) {
        return true;
    ...
    minexclusive: function(value, minValue) {
      ...
      var numberValue = helper.convertToNumber(value);
      var minNumberValue = helper.convertToNumber(minValue);

      return numberValue > minNumberValue;
    },
    mininclusive: function(value, minValue) {
      if (!value) {
        return true;
...
$(document).ready(function() {
  ...
  var typesSelector = Object.keys(inputTypes)
    .map(function(key) {
      return ".needs-validation [data-input-type=" + key + "]";
    })
    .join(", ");

  $(typesSelector).each(function(e) {
    var $this = $(this);
    ...
    for (var i = 0; i < this.attributes.length; i++) {
      ...
      if (
        !(attribute.name.substr(0, 14) === "data-validate-") ||
        inputTypes[type][attribute.name.substr(14)] === null
      ) {
        continue;
      }
      var name = attribute.name.substr(14);
      validators.push({
        name: name,
        func: inputTypes[type][name],
        arg: attribute.value
      });
    }
    this.validators = validators;

    var eventType = $this.data("validation-event") || "keyup";

    $this.on(eventType, function() {
      var value = this.value.trim();
      var type = this.attributes["data-input-type"].value;
      var valid = inputTypes[type]["validate"](value);

      if (valid) {
        this.validators.forEach(function(validator) {
          valid = valid && validator.func(value, validator.arg);
        });
      }

      this.setCustomValidity(valid ? "" : "Ugyldig værdi");
    });
  });
});
...
var helper = {
  ...
  checkInteger: function(value) {
    ...
    var numberValue = helper.convertToNumber(value);
    if (value !== "" && value !== "-" && value !== "+" && isNaN(numberValue)) {
      return false;
    } else {
      return true;
    }
  },
  checkDkDecimal: function(value) {
    var numberValue = helper.convertToNumber(value); //Convert to valid javascript number
    if (value !== "" && value !== "-" && value !== "+" && isNaN(numberValue)) {
      return false;
    }
  ...
  convertToDate: function(value) {
    ...
    if (
      ...
      !helper.checkInteger(components[0]) ||
      components[1].length !== 2 ||
      !helper.checkInteger(components[1]) ||
      components[2].length !== 4 ||
      !helper.checkInteger(components[2])
    ) {
      return null;
    }
    ...
    return Number(components[2]) === date.getFullYear() &&
      Number(components[1]) === date.getMonth() + 1 &&
      Number(components[0]) === date.getDate()
      ? date
      : null;
  }
};
...
$(document).ready(function() {
  // Vis/Skjul med radio buttons
  $("[data-radio-toggle='show']").change(function() {
    $("[name=" + this.attributes["name"].value + "]").each(function() {
      toggleTarget(this);
    });
  });

  // Vis/Skjul med checkboxes
  $("[data-check-toggle='show']").change(function() {
    toggleTarget(this);
  });

  $(".styrelsesheader__infotoggler").click(function() {
    $(this)
      .parent()
...
function toggleTarget(element) {
  var targetAttr = element.attributes["data-target"];
  if (!targetAttr) {
    return;
  }
  ...
  target[checked ? "removeClass" : "addClass"]("d-none");
}
...
;(function() {
  ...
  Environment.isiOS = navigator.userAgent.match(/(iPad|iPhone|iPod)/i)
    ? true
    : false
...
;(function(jQuery) {
  ...
  function formatCodes(locale, isFullLocale) {
    ...
    if (isFullLocale == false) {
      ...
      if (locale.indexOf('_') != -1) locale = locale.split('_')[1].toLowerCase()
